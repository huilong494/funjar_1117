...
.prepareStatement("SELECT A.EID,  C.NAME,  A.YEAR,  B.KNAME,  A.CREDIT FROM HOURS A,  PSE_KIND B,  EMPLOYEE C WHERE A.EID  = C.EID AND B.KID  = A.KID AND (A.YEAR = ? OR ? IS NULL) AND (A.EID  = ? OR ? IS NULL) AND (C.NAME = ? OR ? IS NULL)"
	...
	public List<UseLeft> SearchUseHour(String status,String year){
		...
		try {
			...
			conn = makeConnection();
			PreparedStatement pstmt = conn
.prepareStatement("SELECT A.EID,C.KID,SUM(C.PCTOTAL),D.KNAME FROM EMPLOYEE A ,PSE_MAIN B ,PSE_SUB C,PSE_KIND D WHERE A.EID=B.EID AND B.PID=C.PID AND C.KID=D.KID AND STATUS="+status+" AND C.STARTDATETIME BETWEEN STR_TO_DATE('"+year+"-01-01','%Y-%m-%d') AND STR_TO_DATE('"+year+"-12-31','%Y-%m-%d') GROUP BY A.EID,C.KID,D.KNAME");

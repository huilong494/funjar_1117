...
public class JDBCIndex2 extends JDBCCore {
	...
	public List<RemainHour> remainHour(String eid) {
		...
		try {
			...
			PreparedStatement pstmt = conn
					.prepareStatement("select b.credit-sum(a.pctotal),e.kname,e.kid from PSE_SUB a, Hours b ,PSE_main c,employee d,PSE_Kind e where b.eid=? and a.Kid=b.Kid and b.year=date_format(sysdate, '%Y')  and c.pid=a.pid and c.eid=b.eid and d.eid=b.eid and e.kid=a.kid and a.kid!=1 group by b.credit , d.name,e.kname,e.kid order by e.kid desc");
	...
	public List<RemainHour> getcredit(String eid) {
		...
		try {
			...
			conn = makeConnection();
			PreparedStatement pstmt = conn
					.prepareStatement("select a.CREDIT,b.kname from Hours a,PSE_KIND b where a.Eid=? and a.kid=b.kid  and a.year=date_format(sysdate, '%Y') order by a.kid asc");
			pstmt.setString(1, eid);

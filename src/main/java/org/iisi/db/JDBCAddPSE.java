	...
	public int nPSE(String Eid, String ad) {
		...
		try {
			...
			PreparedStatement st2 = conn
					...
					.prepareStatement("insert into PSE.PSE_MAIN values(?,?,3,STR_TO_DATE(?,'YYYY/MM/DD HH24/MI'),null)");
	...
	public int nSubPSE(int Pid,int Pcid, String pk,String ds, String de,
		...
		try {
			...
			PreparedStatement st = conn
					...
					.prepareStatement("insert into PSE.PSE_SUB values(?,?,STR_TO_DATE(?,'%Y-%m-%d%H:%i'),STR_TO_DATE(?,'%Y-%m-%d%H:%i'),?,?,?)");

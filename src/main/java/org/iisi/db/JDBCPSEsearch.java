...
int dd=Integer.parseInt(dep);
		...
		try {
			...
			if (emp == null && ename == null && sdate == null && edate == null
				...
				pstmt = conn
						.prepareStatement("select date_format(main.applytime, '%Y-%m-%d'),main.eid,emp.name,main.pid,kind.kname,date_format(sub.startdatetime, '%Y-%m-%d %H-%i'),date_format(sub.enddatetime, '%Y-%m-%d %H-%i'),status.sname from employee emp,pse_main main , pse_sub sub, pse_kind kind,pse_status status where emp.eid=main.eid and main.pid=sub.pid and sub.kid=kind.kid and main.status=status.status and emp.dep_id=? order by main.applytime desc");
				pstmt.setInt(1, dd);

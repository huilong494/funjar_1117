   ...
   int dd=Integer.parseInt(dep);
	...
	public List<CkeckSub> ListPSE(int pid) {
		...
		try {
			...
			Connection conn;
			conn = (Connection) makeConnection();
			PreparedStatement pstmt = (PreparedStatement) conn.prepareStatement("select b.pcid,c.kname,b.pctotal,date_format(b.startdatetime,'%Y-%m-%d'),time_format(b.startdatetime,'%H:%i'),date_format(b.enddatetime,'%Y-%m-%d'),time_format(b.enddatetime,'%H:%i'),b.ps from pse_main a,pse_sub b ,pse_kind c where a.pid=b.pid and b.kid=c.kid and a.pid=?");
			pstmt.setInt(1, pid);
			ResultSet rs = pstmt.executeQuery();
			while (rs.next()) {
